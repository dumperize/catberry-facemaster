...
function StoreCatalog(name) {
    ...
    if (this.nameCatalog == 'article' || this.nameCatalog == 'video') {
        this._options.data.order = 'date DESC';
        this._options.data.expand = 'owner';
    }
}
...
StoreCatalog.prototype.load = function () {
    console.time('StoreCatalog - load');
    var self = this;
    return self.$context.sendAction("Paginator", "getCurrentPage")
        ...
        .then(function (rubrikator) {
            ...
            return self._getData(rubrikator.list, rubrikator.active)
                .then(function (data) {
                    console.timeEnd('StoreCatalog - load');
                    return data;
                });
        });
};
